...
if (isset($_POST['signup'])) {
    ...
    $name = $_POST['name']; // get posted data and remove whitespace
    $email = pg_escape_string($_POST['email']);
    $upass = pg_escape_string($_POST['pass']);

    // hash password with SHA256;
    $password = hash('sha256', $upass);

    // check email exist or not
    $result = pg_query($conn,"SELECT email FROM users WHERE email='$email'");
 /*   $stmt->pg_execute($conn,"my_query",'$stmt');
    ...
    if ($count == 0) { // if email is not found add user
        ...
        $stmts = pg_query($conn,"INSERT INTO users(name,email,password) VALUES('$name', '$email', '$password') RETURNING user_id");
       /* $stmts->bind_param("sss", $uname, $email, $password);
        $res = $stmts->execute();//get result
        $stmts->close();  */
		$insert_row = pg_fetch_row($stmts);
$user_id = $insert_row[0];
        ...
        <form method="post" autocomplete="off">
            ...
            <div class="col-md-12">
                ...
                <div class="form-group">
                    <h2 class="">Register for our System</h2>
                </div>
